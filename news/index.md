...
- **Roxygen markdown is enabled, which changes every help page.**
  ...
  Rd uninterpreted. Regenerating the Rd files turned that markdown into
  Rd markup throughout: code spans that had been printing as literal
  backticks, and bold and italic that had been printing as their own
  asterisks. The topics also gained cross-references, of which there had
  been none: each family links to its `_lccdf_stanvars()` companion and
  to the families it is meant to be fitted against.
